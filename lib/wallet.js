        ...
        .then((complete) => {
          ...
          _.forEach(complete, (value) => {
            ...
            attachmentText += 'Start: ' + parseFloat(value.start).formatISK(2, 3, '.', ',')
              ...
              + '\nIn: ' + parseFloat(value.in).formatISK(2, 3, '.', ',')
              + '\nOut: ' + parseFloat(value.out).formatISK(2, 3, '.', ',')
              + '\nDiff-Cashflow: ' + parseFloat(value.in - value.out).formatISK(2, 3, '.', ',');
      ...
      self.cache.getClient().hgetall(cacheKey,  (err, result) => {
        ...
        _.forEach(result, (value) => {
          ...
          if (_.inRange(journalDate, period.start, period.end)) {
            ...
            const amount = parseFloat(_data.amount);
            if (amount > 0) {
              current.in += amount;
            } else {
